// Réplica de backend/src/domain/pricing (ADR-08): el servidor es la fuente
// autoritativa; esta copia solo alimenta la proyección en vivo del simulador.
import { PRICING_TIERS } from './tiers'
import type { CostComponent, PricingInput, PricingResult, TierLine } from './types'
...
export function computeUserTierLines(activeUsers: number): TierLine[] {
  ...
  for (const { tier, fromUser, toUser, unitCents } of PRICING_TIERS) {
    ...
    const lastUser = toUser === null ? activeUsers : Math.min(toUser, activeUsers)
